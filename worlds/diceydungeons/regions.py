...
def create_all_regions(world: DiceyDungeonsWorld) -> None:
    ...
    regions = [menu]
    # Region name: <Character> - Episode <Episode number> - Floor <Floor number>
    for _, character in CHARACTER_CODE_TO_NAME.items():
        for episode in ["Episode 1", "Episode 2", "Episode 3", "Episode 4", "Episode 5", "Episode 6"]:
            for floor in ["Floor 1", "Floor 2", "Floor 3", "Floor 4", "Floor 5", "Floor 6"]:
                region = Region(character + " - " + episode + " - " + floor, world.player, world.multiworld)
...
def connect_regions(world: DiceyDungeonsWorld) -> None:
    ...
    for _, character in CHARACTER_CODE_TO_NAME.items():
        for episode in ["Episode 1", "Episode 2", "Episode 3", "Episode 4", "Episode 5", "Episode 6"]:
            region = world.get_region(character + " - " + episode + " - Floor 1")
    ...
    for _, character in CHARACTER_CODE_TO_NAME.items():
        for i in range(1, len(floors)):
            for episode in ["Episode 1", "Episode 2", "Episode 3", "Episode 4", "Episode 5", "Episode 6"]:
                pre = world.get_region(character + " - " + episode + " - " + floors[i - 1])
                post = world.get_region(character + " - " + episode + " - " + floors[i])
